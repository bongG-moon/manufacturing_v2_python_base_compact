...
def extract_columns(data: List[Dict[str, Any]]) -> List[str]:
    # Build a stable column list from row dictionaries.
    columns: List[str] = []
    for row in data:
        for key in row.keys():
            name = str(key)
...
def dataset_profile(data: List[Dict[str, Any]]) -> DatasetProfile:
    # The LLM sees this profile instead of the full dataset.
    return {
...
def find_requested_dimensions(query_text: str) -> List[str]:
    # Detect analysis axes like MODE별, 라인별, 공정군별 from the question text.
...
def find_missing_dimensions(query_text: str, columns: List[str]) -> List[str]:
    # If a requested axis is not in the current table, fail early with a clear message.
...
def minimal_fallback_plan(query_text: str, data: List[Dict[str, Any]]) -> PreprocessPlan:
    # This is the last-resort path when LLM code generation is unavailable.
    columns = extract_columns(data)
...
def validate_plan_columns(plan: PreprocessPlan, columns: List[str]) -> List[str]:
    # The LLM may mention columns in plan metadata. We verify them before execution.
...
def build_transformation_summary(
    ...
    # The UI uses this to explain how the table changed after follow-up analysis.
    return {
